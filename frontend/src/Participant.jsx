...
export default function Participant({ userAddress }) {
  ...
  const handleBulkRevoke = async () => {
    if (selectedConsents.size === 0){
      setError("Please select at least one consent to revoke");
      return;
    }
    try {
      setTxLoading(true);
      ...
      const txs = [];
      for(const id of selectedConsents){
        const tx = await contract.revokeConsent(Number(id));
        txs.push(tx);
      }
      ...
      setError(err.message);
      console.error("Error revoking consents:", err);
    }finally{
      setTxLoading(false);
    }
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-2">
            ...
            {selectedConsents.size > 0 && (
              <Badge>{selectedConsents.size} selected</Badge>
            )}
          </div>
          </CardHeader>
          <CardContent>
          {activeConsents.length === 0 ? (
            ...
            <ul className="space-y-3">
              {activeConsents.map((consent) => {
                const isSelected = selectedConsents.has(consent.id);
                return (
                    <li
                    key={consent.id}
                  ...
                  >
                    <Checkbox
                      checked={isSelected}
                      onCheckedChange={() => toggleConsentSelection(consent.id)}
                      disabled={txLoading}
                      className="mt-0.5"
                      aria-label={`Select consent from ${consent.requester_name}`}
                    />
        ...
        {activeConsents.length > 0 && (
          <div className="flex items-center justify-end gap-2 border-t px-6 pt-4">
            <Button
              ...
              disabled={selectedConsents.size === 0 || txLoading}
